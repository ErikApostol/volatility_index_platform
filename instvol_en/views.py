...
import instvol_en.myplotly
import pickle
import csv

from instvol_en.ivol import ivol
from instvol_en.hvol import hvol
from instvol_en.hisrho import hisrho
from instvol_en.insrho import insrho
from instvol_en.getcorr import getcorr
from instvol.models import * # Changing to instvol causes "Stock matching query does not exist."
#from instvol_en.VIXmap import tickers
from instvol_en.VIXmap import *
...
def engine(request):
    if request.method == 'POST': #custom upload
        ...
        if form.is_valid():
            ...
            div = instvol_en.myplotly.getdiv(cusdata.index, cusdata.iloc[:,0].astype('double'),
        ...
        div = instvol_en.myplotly.getdiv(fdata.index, fdata['price'].astype('double'), 100*inst_vol, hist_vol, ewma_vol, garch_vol, vdata, symbol, True)
        form = UploadFileForm()

    rnd_str = str(randint(1000000000,10000000000))
    csv_filename = 'instvol_en/results_csv/' + rnd_str + '.csv'
    df.to_csv('instvol_en/static/' + csv_filename)
...
def correlation(request):
    if request.method == 'POST': #custom upload
        ...
        instcorrdiv = instvol_en.myplotly.getcorrdiv(rhodata)
        hiscorrdiv = instvol_en.myplotly.getcorrdiv(rhodata, isHis=1)
        spotdiv = instvol_en.myplotly.getspotdiv(spotdata)
